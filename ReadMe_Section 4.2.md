...
- It also describes the following summary files:
  - `99_SummaryAll_3_Simulation 4.2_confounding.R`
...
1. Simulation files
   ...
   - Hyperparameters the user needs to specify:
     ...
     - `nSim`: The number of simulation replicates.
       - For Section 4.2, `nSim = 100` is used for *decomp knockoffs*, and `nSim = 20` is used for the BH procedure.
...
2. Summary files
   - `99_SummaryAll_3_Simulation4.2_confounding.R`
     - This code reproduces Tables 1 and 2 in Section 4.2.
     - This code consists of two parts:
       - Part for Table 1 (*decomp knockoffs*)
       - Part for Table 2 (BH procedure)

   - The current working directory must be set to the directory where the `.rda` files are stored.
     - For the Table 1 and Table 2 parts, the working directory must be set differently.
   ...
   - `target_FDR_set`: A scalar specifying the target FDR value.
     - For Tables 1 and 2, `target_FDR_set = 0.1`.
   ...
   - For each method and setting, the average FDP, the standard deviation of the FDP, the average power, and the standard deviation of the power are stored in four separate `.csv` files:
     - `Section42_Table1_LR_nSim100_DownNULL_Avg_FDP.csv`: Average FDP
     - `Section42_Table1_LR_nSim100_DownNULL_SD_FDP.csv`: SD of FDP
     - `Section42_Table1_LR_nSim100_DownNULL_Avg_Power.csv`: Average power
     - `Section42_Table1_LR_nSim100_DownNULL_SD_Power.csv`: SD of power
