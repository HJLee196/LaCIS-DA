...
for(nsim in 1:nSim){
  ...
  if (testUse != "LCD")
  {
    ...
    # xp_data.matrix %>% apply(2, sd) %>% head()
    # xp_data.matrix_sd %>% head()
  
    xp_data.full = 
      xp_data.matrix %>%
      sweep(MARGIN = 2, STATS = xp_data.matrix_sd, FUN = "/")
    # xp_data.full %>% apply(2, sd) %>% head()
    ...
    # cv_lasso_fit_coef_orig = cv_lasso_fit_coef[rownames(cv_lasso_fit_coef) %in% colnames(xp_data.matrix),]
    ...
    for (m_ko in 1:m_kos)
    {
      ...
      # cv_lasso_fit_coef_mth_ko = cv_lasso_fit_coef[rownames(cv_lasso_fit_coef) %in% colnames_mth_ko,]
